...
if ($new_pass != $new_pass1) {
    header('location: settingpage.php?error=The two passwords don\'t match');
} else {
    if ($old_pass == $orig_pass) {
        $query = "UPDATE  users SET password = '" . $new_pass . "' WHERE email = '" . $_SESSION['email'] . "'";
        mysqli_query($con, $query) or die($mysqli_error($con));
        header('location: settingpage.php?error=Password Updated');
    } else
        header('location: settingpage.php?error=Wrong Old Password');
}
?>